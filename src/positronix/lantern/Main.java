...
public class Main extends Application  {
	...
	*/
	...
	public static void newFile() {
			...
			for(int i = 0; i < info.size(); i++){
				...
				try {
					if(infoObjects.get(i) instanceof TextField){
						...
						((ListView) infoObjects.get(i)).getItems().clear();
					}	
				} catch (NumberFormatException e) {
					e.printStackTrace();
				}
		...
		loadValues();
		Dialog.generated = false;
		OverviewTab.generateScores.setDisable(false);
		OverviewTab.generateScores.setVisible(true);
		OverviewTab.race.setDisable(false);
		OverviewTab.firstTime = true;
		ClassesTab.addLevels.setDisable(true);
		ClassesTab.classChoice.getSelectionModel().select(0);
		EquipmentTab.notEnoughGold = false;
		EquipmentTab.tablesPopulated = false;
		ModifiersTab.addMod.setDisable(true);
	}
